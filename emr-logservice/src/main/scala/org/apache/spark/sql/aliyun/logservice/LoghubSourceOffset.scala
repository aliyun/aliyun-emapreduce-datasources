...
case class LoghubShardOffset(logProject: String, logStore: String, shard: Int, offset: Int, cursor: String) extends PartitionOffset

object LoghubSourceOffset {
  private implicit val formats = Serialization.formats(NoTypeHints)

  def getShardOffsets(offset: Offset, sourceOptions: Map[String, String]): Map[LoghubShard, (Int, String)] = {
    offset match {
  ...
  def partitionOffsets(str: String, sourceOptions: Map[String, String]): Map[LoghubShard, (Int, String)] = {
    ...
    try {
      Serialization.read[Map[String, Map[Int, Int]]](str).flatMap { case (log, shardOffset) =>
        shardOffset.map { case (shard, offset) =>
          ...
          val cursor = logServiceClient.GetCursor(logProject, logStore, shard, offset.toLong).GetCursor()
  ...
  def apply(shardToOffsets: Map[LoghubShard, Int], sourceOptions: Map[String, String]): LoghubSourceOffset = {
    val logServiceClient: LoghubClientAgent = LoghubOffsetReader.getOrCreateLoghubClient(sourceOptions)
  ...
  def apply(offsetTuples: (String, String, Int, Int, String)*): LoghubSourceOffset = {
    LoghubSourceOffset(offsetTuples.map { case (p, ls, sh, os, cursor) => (LoghubShard(p, ls, sh), (os, cursor))}.toMap)
  }
