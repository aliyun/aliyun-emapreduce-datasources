...
import org.apache.spark.sql.aliyun.datahub.DatahubOffsetReader._
...
class DatahubContinuousReader(
  ...
  override def createUnsafeRowReaderFactories(): util.List[DataReaderFactory[UnsafeRow]] = {
    ...
    startOffsets.toSeq.map {
      case (loghubShard, of) => {
        DatahubContinuousDataReaderFactory(loghubShard.project, loghubShard.topic, loghubShard.shardId, of, sourceOptions)
