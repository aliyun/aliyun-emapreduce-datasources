...
DROP TABLE IF EXISTS kafka_temp_table_q8;

SET spark.testing=${TESTING_ENABLE};
SET spark.sql.streaming.query.timeout.ms=${TESTING_TIMEOUT_MS};
SET streaming.query.name=job8;
SET spark.sql.streaming.checkpointLocation.job8=${CHECKPOINT_ROOT}/job8;

CREATE TABLE kafka_temp_table_q8
USING kafka
OPTIONS (
kafka.bootstrap.servers = "${BOOTSTRAP_SERVERS}",
subscribe = 'temp_topic_q8',
...
INSERT INTO kafka_temp_table_q8
SELECT
  s_store_name,
  sum(ss_net_profit)
FROM kafka_store_sales, date_dim, store,
  (SELECT ca_zip
  FROM (
