...
public class JetOssNativeFileSystemStore implements NativeFileSystemStore {
  private static final Log LOG =
      LogFactory.getLog(JetOssNativeFileSystemStore.class);
  ...
  public void initialize(URI uri, Configuration conf) throws Exception {
    ...
    if (endpoint == null) {
      endpoint = EndpointEnum.getEndpoint("oss",
          MetaClient.getClusterRegionName(), MetaClient.getClusterNetworkType());
      if (endpoint == null) {
        throw new IllegalArgumentException("Can not find any suitable " +
            "endpoint, you can set it in OSS URI");
      }
    }

    if (securityToken == null) {
      this.ossClient =
          new OSSClientAgent(endpoint, accessKeyId, accessKeySecret, conf);
    } else {
      this.ossClient =
          new OSSClientAgent(endpoint, accessKeyId, accessKeySecret,
              securityToken, conf);
    }
    this.numCopyThreads =
        conf.getInt("fs.oss.uploadPartCopy.thread.number", 10);
    this.numPutThreads =
        conf.getInt("fs.oss.uploadPart.thread.number", 5);
    this.maxSplitSize =
        conf.getInt("fs.oss.multipart.split.max.byte", 5 * 1024 * 1024);
    this.numSplits =
        conf.getInt("fs.oss.multipart.split.number", 10);
    this.maxSimpleCopySize =
        conf.getLong("fs.oss.copy.simple.max.byte", 64 * 1024 * 1024L);
    this.maxSimplePutSize =
        conf.getLong("fs.oss.put.simple.max.byte", 5 * 1024 * 1024);
  }
    ...
    try {
      if (!append) {
        ...
        if (fileLength < Math.min(maxSimplePutSize, 512 * 1024 * 1024L)) {
          ...
          LOG.info("using multipart upload for key " + key + ", size: " +
              fileLength);
  ...
  @Override
  public void storeFiles(String key, List<File> files, boolean append)
      throws IOException {
    try {
      if (!append) {
        if (files.size() == 1 && files.get(0).length() <
            Math.min(maxSimplePutSize, 512 * 1024 * 1024L)) {
          ...
          LOG.info("using multipart upload for key " + key + ", " +
              "block files: " + sb.toString());
  ...
  public InputStream retrieve(String key) throws IOException {
    try {
      ...
      ObjectMetadata objectMetadata = ossClient.getObjectMetadata(bucket, key);
      OSSObject object = ossClient.getObject(bucket, key, 0,
          objectMetadata.getContentLength(), conf);
  ...
  public InputStream retrieve(String key, long byteRangeStart)
    ...
    try {
      ...
      OSSObject object = ossClient.getObject(bucket, key, byteRangeStart,
          fileSize - 1, conf);
      return object.getObjectContent();
    } catch (Exception e) {
      handleException(key, e);
  ...
  public InputStream retrieve(String key, long byteRangeStart, long length)
      throws IOException {
    try {
      if (!doesObjectExist(key)) {
        return null;
      ...
      OSSObject object =
          ossClient.getObject(bucket, key, byteRangeStart, end, conf);
      return object.getObjectContent();
    } catch (Exception e) {
      handleException(key, e);
      return null; //never returned - keep compiler happy
  ...
  public PartialListing list(String prefix, int maxListingLength,
    ...
    return list(prefix, recurse ? null :
        NativeOssFileSystem.PATH_DELIMITER, maxListingLength, priorLastKey);
  }
  ...
  private PartialListing list(String prefix, String delimiter,
    ...
    try {
      if (prefix.length() > 0
          && !prefix.endsWith(NativeOssFileSystem.PATH_DELIMITER)) {
      ...
      ObjectListing listing = ossClient.listObjects(bucket, prefix, delimiter,
          maxListingLength, priorLastKey, conf);
      ...
      return new PartialListing(listing.getNextMarker(), fileMetadata,
          listing.getCommonPrefixes().toArray(new String[0]));
    } catch (Exception e) {
      handleException(e);
      return null; //never returned - keep compiler happy
    }
  ...
  public void copy(String srcKey, String dstKey) throws IOException {
    try {
      ...
      if (contentLength <= Math.min(maxSimpleCopySize, 512 * 1024 * 1024L)) {
        ...
        LOG.info("using multipart copy for copying from srckey " +
            srcKey + " to " + dstKey + ", size: " + contentLength);
  ...
  public void purge(String prefix) throws IOException {
    try {
      List<OSSObjectSummary> objects =
          ossClient.listObjects(bucket, prefix).getObjectSummaries();
  ...
  public void dump() throws IOException {
    ...
    try {
      List<OSSObjectSummary> objects =
          ossClient.listObjects(bucket).getObjectSummaries();
      for (OSSObjectSummary ossObjectSummary : objects) {
  ...
  private void handleException(String key, Exception e)
    ...
    if (e instanceof ServiceException
        && "NoSuchKey".equals(((ServiceException) e).getErrorCode())) {
  ...
  private void doMultipartCopy(String srcKey, String dstKey, Long contentLength)
    ...
    Long partSize = Math.max(Math.min(maxSplitSize, contentLength / numSplits),
        minSplitSize);
    ...
    for (int i = 0; i < partCount; i++) {
      ...
      OSSCopyTask ossCopyTask = new OSSCopyTask(
          ossClient, uploadId, bucket, bucket, srcKey, dstKey, size,
          skipBytes, i + 1, conf);
    ...
    try {
      ...
      for (int i = 0; i < partCount; i++) {
        UploadPartCopyResult uploadPartCopyResult = (UploadPartCopyResult)
            ((Result) responseMap.get(i + "")).getModels()
                .get("uploadPartCopyResult");
  ...
  private void doMultipartPut(File file, String key) throws IOException {
    ...
    for (int i = 0; i < partCount; i++) {
      ...
      long size = partSize < contentLength - skipBytes ? partSize :
          contentLength - skipBytes;
      OSSPutTask ossPutTask = new OSSPutTask(ossClient, uploadId, bucket, key,
          size, skipBytes, i + 1, file, conf);
  ...
  private void doMultipartPut(List<File> files, String key) throws IOException {
    ...
    Long partSize =
        Math.max(Math.min(maxSplitSize, totalContentLength / numSplits),
            minSplitSize);
