...
public class OssInputUtils {
  ...
  public FileSplit[] getSplits(String file, int numSplits) throws IOException {
    ...
    long minSize = Math.max(conf.getLong(org.apache.hadoop.mapreduce.lib.
        input.FileInputFormat.SPLIT_MINSIZE, 1), 1);
    ...
    for (FileStatus file2 : files) {
      ...
      if (length != 0) {
        ...
        while (((double) bytesRemaining) / splitSize > SPLIT_SLOP) {
          FileSplit split = new FileSplit(fp, length - bytesRemaining,
              splitSize, new String[0]);
          splits.add(split);
          bytesRemaining -= splitSize;
        }
        if (bytesRemaining != 0) {
          FileSplit split = new FileSplit(fp, length - bytesRemaining,
              bytesRemaining, new String[0]);
          splits.add(split);
        }
      }
    }
  ...
  public RecordReader<LongWritable, Text> getOssRecordReader(FileSplit fileSplit,
